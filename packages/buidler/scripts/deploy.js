...
async function main() {
  ...
  const Distributor = await deploy("Distributor",[YourToken.address]);
  const Transactions = await deploy("Transactions",[YourToken.address]);
  const Giveaway = await deploy("Giveaway",[YourToken.address]);
